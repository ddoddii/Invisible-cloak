...
def create_backgroud(cap, num_frames=30):
    print("Creating background, Move out of frame")
    backgrounds = []
    for i in range(num_frames):
        ...
        if ret:
            ...
            print(f"Warning : Coud not read frame {i+1}/{num_frames}")
        time.sleep(0.1)
    if backgrounds:
        return np.median(backgrounds, axis=0).astype(np.uint8)
    else:
        raise ValueError("Could not capture background")
...
def create_mask(frame, lower_color, upper_color):
    ...
    mask = cv2.morphologyEx(
        mask, cv2.MORPH_DILATE, np.ones((3, 3), np.uint8), iterations=2
    )
    return mask
...
def main():
    ...
    if not cap.isOpened():
        ...
        return
    try:
        background = create_backgroud(cap)
